    ...
    with st.container(border=True):
        ...
        for week in cal.monthdayscalendar(year, month):
            ...
            for c, day in zip(cols, week):
                ...
                day_bookings = get_day_bookings(this_date)
                is_selected = this_date == st.session_state.selected_date
                btn_type = "primary" if is_selected else "secondary"
                with c:
                    if st.button(str(day), key=f"day_{year}_{month}_{day}", type=btn_type,
                                 width="stretch"):
                        st.session_state.selected_date = this_date
                        st.rerun()
                    if day_bookings:
                        _render_day_bookings(day_bookings)
...
def _select_advisor(name):
    """Pick a single advisor: move the checkmark and rebuild any pending email draft."""
    st.session_state.selected_advisor = name
    draft = st.session_state.get("draft_email") or {}
    if (draft.get("advisor") and not _same_person(draft["advisor"], name)
            and not st.session_state.get("email_sent")):
        st.session_state.draft_email = {**draft, "advisor": name}
...
def render_book_consultation():
    ...
    st.markdown("<p class='muted'>Pick a slot, or compose a consultation request email to a professor.</p>",
                unsafe_allow_html=True)
    st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)

    # Jump straight to a pre-built draft when one exists (chat redirect path).
    if st.session_state.get("draft_email") and "email_mode" not in st.session_state:
        prepare_email_draft(build_draft())

    tabs = st.tabs(["Schedule", "Email Draft"], key="book_consultation_tabs")
    with tabs[0]:
